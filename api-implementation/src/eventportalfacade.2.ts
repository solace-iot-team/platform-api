...
export class EventPortalfacade {
  ...
  public async getApplicationDomains(pageSize: number, pageNumber: number): Promise<ApplicationDomainList> {
    ...
    try {
      const applicationDomains: ApplicationDomainsResponse = await this.applicationDomainsService.getApplicationDomains(isNaN(pageSize)|| pageSize>100 ? 100 : pageSize, isNaN(pageNumber) ? 1 : pageNumber); if (applicationDomains.data && applicationDomains.data.length > 0) {
