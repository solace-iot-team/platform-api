...
export default class RdpTask extends SEMPv2Task {
    protected paths: string[] = ['attributes', 'tags','services', 'queues', 'queueBindings','restConsumers', 'environments'];
    ...
    protected async create(): Promise<TaskResult> {
        const config: TaskConfigAlias = this.config() as TaskConfigAlias;
        try {
            const response: TaskServiceResponse = await this.apiClient.createMsgVpnRestDeliveryPoint(config.environment.service.msgVpnName, _.omit(config.configObject, this.paths) as TaskServiceRequest);
