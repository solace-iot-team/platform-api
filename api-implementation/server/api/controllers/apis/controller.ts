...
export class Controller {
   ...
   */
     metaAttributeByName(req: Request, res: Response, next: NextFunction): void {
      ApisService.metaAttributeByName(req.params['name'], req.params['attributeName'])
        .then((r) => {
          res.status(200).json(r);
        })
        .catch((e) => {
    ...
    createMetaAttribute(req: Request, res: Response, next: NextFunction): void {
      ApisService.createMetaAttribute(req.params['name'], req.params['attributeName'], req.body).then((r) => {
        if (r) {
          res.status(200).json(r);
        }
        else
    ...
    updateMetaAttribute(req: Request, res: Response, next: NextFunction): void {
      ApisService.updateMetaAttribute(req.params['name'], req.params['attributeName'], req.body).then((r) => {
        if (r) {
          res.status(200).json(r);
        }
        else
    ...
    deleteMetaAttribute(req: Request, res: Response, next: NextFunction): void {
      ApisService.deleteMetaAttribute(req.params['name'], req.params['attributeName']).then((r) => {
        res.status(r).send();
      }).catch((e) => next(e));
    }
