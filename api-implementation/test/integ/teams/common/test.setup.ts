...
/**
 * Registers `before()` and `beforeEach()` hooks for a teams test suite.
 * 
 * The `before()` hook logs a ">>> Start to execute test cases" message and all
 * environment variables that are used, and creates the {@link organization}.
 * 
 * The `beforeEach()` hook generates a new identifier for the {@link TestContext}
 * and configures the {@link PlatformAPIClient} to use the API user.
 * 
 * **Important:**
 * 
 * If the title of the parent test suite matches the start of the title of the teams
 * test suite, the hooks will be registered for the parent test suite instead.
 * 
 * This improves the test execution time when teams tests from multiple test suites
 * are executed.
 * 
 * @param suite The teams test suite.
 */
...
/**
 * Registers an `after()` hook for a teams test suite.
 * 
 * The `after()` hook deletes the {@link organization} (and all resources that are
 * part of it) and logs a ">>> Finished" message.
 * 
 * **Important:**
 * 
 * If the title of the parent test suite matches the start of the title of the teams
