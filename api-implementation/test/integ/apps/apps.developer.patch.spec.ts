...
describe("apps", function () {
  describe(scriptName, function () {
    ...
    it("should update the display name", async function () {
      ...
      const response = await AppsService.updateDeveloperApp(setup.organizationName, developerName, application.name, applicationPatch).catch((reason) => {
    ...
    it("should update the ACL profile when API products are added", async function () {
      ...
      await AppsService.updateDeveloperApp(setup.organizationName, developerName, application.name, applicationPatch).catch((reason) => {
        expect(reason, `error=${reason.message}`).is.instanceof(ApiError);
        expect.fail(`failed to update developer application; error="${reason.body.message}"`);
      });
    ...
    it("should update the ACL profile when attributes are changed", async function () {
      ...
      await AppsService.updateDeveloperApp(setup.organizationName, developerName, application.name, applicationPatch1).catch((reason) => {
        expect(reason, `error=${reason.message}`).is.instanceof(ApiError);
        expect.fail(`failed to update developer application; error="${reason.body.message}"`);
      });
      ...
      await AppsService.updateDeveloperApp(setup.organizationName, developerName, application.name, applicationPatch2).catch((reason) => {
        expect(reason, `error=${reason.message}`).is.instanceof(ApiError);
        expect.fail(`failed to update developer application; error="${reason.body.message}"`);
      });
    ...
    afterEach(async function () {
      await AppsService.deleteDeveloperApp(setup.organizationName, developerName, applicationName);
    });
