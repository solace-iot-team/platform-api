...
describe(`${scriptName}`, () => {
  ...
  context(`${scriptName}`, () => {
    ...
    it(`${scriptName}: should add http protocol to api product maintenance-development`, async () => {
      ...
      try {
        responsePatch = await ApiProductsService.updateApiProduct(orgName, apiProductNameMaintenanceDevelopment, null, requestPatch);
      } catch (e) {
        expect(isInstanceOfApiError(e), `${TestLogger.createNotApiErrorMesssage(e.message)}`).to.be.true;
    ...
    xit(`${scriptName}: should add webhook to development app for all developers`, async () => {
      ...
      for (let developer of developers) {
        ...
        try {
          responsePatch = await AppsService.updateDeveloperApp(orgName, developer.userName, appName, null, requestPatch);
        } catch (e) {
          expect(isInstanceOfApiError(e), `${TestLogger.createNotApiErrorMesssage(e.message)}`).to.be.true;
    ...
    it(`${scriptName}: should approve all apps that require approval`, async () => {
      ...
      for (let app of responseList) {
        ...
        try {
          responsePatch = await AppsService.updateDeveloperApp(orgName, app.ownerId, app.name, null, requestPatch);
        } catch (e) {
          expect(isInstanceOfApiError(e), `${TestLogger.createNotApiErrorMesssage(e.message)}`).to.be.true;
